...
http::HTTPContext::HTTPContext() : method(HTTPMethod::GET), bufferSize(0), index(0) {}

http::HTTPContext::HTTPContext(std::vector<unsigned char> _buffer, size_t _bufferSize) : buffer(std::move(_buffer)), bufferSize(_bufferSize) {
...
http::HTTPContext & http::HTTPContext::operator=(const HTTPContext &_other) {
    if(&_other == this)
        return *this;
...
std::vector<unsigned char> http::HTTPContext::ToData()  {
...
void http::HTTPContext::SetMethod(std::string &str) {
...
void http::HTTPContext::ParseHeader() {
...
std::string http::HTTPContext::GetLine() {
    std::string ret;
    while(index < bufferSize){
        if(buffer[index] == '\n'){
            ++index;
