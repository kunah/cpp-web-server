...
http::HTTPFileProcess::HTTPFileProcess(std::string _filePath, std::string _contentType, uint32_t _maxAge)
        : BaseHTTPStaticContentProcess(std::move(_filePath), std::move(_contentType)), maxAge(_maxAge) {}

http::HTTPContext http::HTTPFileProcess::Process(http::HTTPContext &request) {
